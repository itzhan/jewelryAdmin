import React, { memo, useEffect, useRef, useState } from 'react';
import { Card, Table, Tag, Link, Button, Dialog, Form, Input, Switch, MessagePlugin } from 'tdesign-react';
import classnames from 'classnames';
import {
  getStoneCertificates,
  StoneCertificateItem,
  createStoneCertificate,
  updateStoneCertificate,
  deleteStoneCertificate,
  StoneCertificatePayload,
} from 'services/backend';
import CommonStyle from 'styles/common.module.less';
import { FormInstanceFunctions, SubmitContext } from 'tdesign-react/es/form/type';

const { FormItem } = Form;

const StoneCertificatesPage = () => {
  const [loading, setLoading] = useState(false);
  const [list, setList] = useState<StoneCertificateItem[]>([]);
  const [formVisible, setFormVisible] = useState(false);
  const [saving, setSaving] = useState(false);
  const [editing, setEditing] = useState<StoneCertificateItem | null>(null);
  const formRef = useRef<FormInstanceFunctions>();

  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await getStoneCertificates();
      setList(data);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchData();
  }, []);

  const handleAdd = () => {
    setEditing(null);
    setFormVisible(true);
  };

  const handleEdit = (record: StoneCertificateItem) => {
    setEditing(record);
    setFormVisible(true);
  };

  const handleDelete = async (record: StoneCertificateItem) => {
    try {
      setLoading(true);
      await deleteStoneCertificate(record.id);
      MessagePlugin.success('删除成功');
      fetchData();
    } catch (e) {
      MessagePlugin.error('删除失败');
    } finally {
      setLoading(false);
    }
  };

  const onSubmit = async (ctx: SubmitContext) => {
    if (ctx.validateResult !== true) return;
    const values = formRef.current?.getFieldsValue?.(true) as StoneCertificatePayload;
    try {
      setSaving(true);
      if (editing) {
        await updateStoneCertificate(editing.id, values);
        MessagePlugin.success('更新成功');
      } else {
        await createStoneCertificate(values);
        MessagePlugin.success('创建成功');
      }
      setFormVisible(false);
      fetchData();
    } catch (e) {
      MessagePlugin.error('保存失败');
    } finally {
      setSaving(false);
    }
  };

  const columns = [
    { colKey: 'id', title: 'ID', width: 80 },
    { colKey: 'displayName', title: '名称', width: 160 },
    { colKey: 'description', title: '描述', width: 180 },
    {
      colKey: 'website',
      title: '官网',
      width: 260,
      cell({ row }: { row: StoneCertificateItem }) {
        if (!row.website) return '-';
        return (
          <Link href={row.website} target='_blank' hover='color'>
            {row.website}
          </Link>
        );
      },
    },
    {
      colKey: 'isActive',
      title: '是否启用',
      width: 120,
      cell({ row }: { row: StoneCertificateItem }) {
        return row.isActive ? (
          <Tag theme='success' variant='light'>
            启用
          </Tag>
        ) : (
          <Tag theme='default' variant='outline'>
            停用
          </Tag>
        );
      },
    },
    {
      colKey: 'createdAt',
      title: '创建时间',
      width: 180,
      cell({ row }: { row: StoneCertificateItem }) {
        return new Date(row.createdAt).toLocaleString();
      },
    },
    {
      colKey: 'updatedAt',
      title: '更新时间',
      width: 180,
      cell({ row }: { row: StoneCertificateItem }) {
        return new Date(row.updatedAt).toLocaleString();
      },
    },
    {
      colKey: 'op',
      title: '操作',
      width: 180,
      cell({ row }: { row: StoneCertificateItem }) {
        return (
          <>
            <Button theme='primary' variant='text' onClick={() => handleEdit(row)}>
              编辑
            </Button>
            <Button theme='danger' variant='text' onClick={() => handleDelete(row)}>
              删除
            </Button>
          </>
        );
      },
    },
  ];

  return (
    <div className={classnames(CommonStyle.pageWithPadding, CommonStyle.pageWithColor)}>
      <Card
        title='石头证书机构表（stone_certificates）'
        bordered={false}
        actions={
          <Button theme='primary' onClick={handleAdd}>
            新增证书机构
          </Button>
        }
      >
        <Table rowKey='id' data={list} columns={columns} loading={loading} />
      </Card>
      <Dialog
        header={editing ? '编辑证书机构' : '新增证书机构'}
        visible={formVisible}
        confirmBtn={{ content: '保存', loading: saving }}
        cancelBtn='取消'
        onClose={() => setFormVisible(false)}
        onConfirm={() => formRef.current?.submit?.()}
      >
        <Form
          ref={formRef}
          labelWidth={80}
          onSubmit={onSubmit}
          colon
          key={editing ? editing.id : 'new'}
        >
          <FormItem
            label='编码'
            name='code'
            initialData={editing?.code}
            rules={[{ required: true, message: '请输入编码', type: 'error' }]}
          >
            <Input placeholder='例如 GIA' />
          </FormItem>
          <FormItem
            label='名称'
            name='displayName'
            initialData={editing?.displayName}
            rules={[{ required: true, message: '请输入名称', type: 'error' }]}
          >
            <Input />
          </FormItem>
          <FormItem label='描述' name='description' initialData={editing?.description}>
            <Input placeholder='可选，用于解释证书机构' />
          </FormItem>
          <FormItem label='官网地址' name='website' initialData={editing?.website}>
            <Input placeholder='例如 https://www.gia.edu' />
          </FormItem>
          <FormItem label='是否启用' name='isActive' initialData={editing?.isActive ?? true}>
            <Switch />
          </FormItem>
        </Form>
      </Dialog>
    </div>
  );
};

export default memo(StoneCertificatesPage);
