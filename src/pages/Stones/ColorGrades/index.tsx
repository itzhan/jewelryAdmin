...
import {
  getStoneColorGrades,
  StoneColorGradeItem,
  createStoneColorGrade,
  updateStoneColorGrade,
  deleteStoneColorGrade,
  StoneEnumPayload,
...
const StoneColorGradesPage = () => {
  const [loading, setLoading] = useState(false);
  const [list, setList] = useState<StoneColorGradeItem[]>([]);
  const [formVisible, setFormVisible] = useState(false);
  const [saving, setSaving] = useState(false);
  const [editing, setEditing] = useState<StoneColorGradeItem | null>(null);
  const formRef = useRef<FormInstanceFunctions>();

  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await getStoneColorGrades();
      setList(data);
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = (record: StoneColorGradeItem) => {
    setEditing(record);
    setFormVisible(true);
  };

  const handleDelete = async (record: StoneColorGradeItem) => {
    try {
      setLoading(true);
      await deleteStoneColorGrade(record.id);
  ...
  const onSubmit = async (ctx: SubmitContext) => {
    ...
    try {
      setSaving(true);
      if (editing) {
        await updateStoneColorGrade(editing.id, values);
        MessagePlugin.success('更新成功');
      } else {
        await createStoneColorGrade(values);
  ...
  const columns = [
    ...
    { colKey: 'displayOrder', title: '排序', width: 100 },
    {
      ...
      title: '是否启用',
      width: 120,
      cell({ row }: { row: StoneColorGradeItem }) {
        return row.isActive ? (
  ...
  return (
    <div className={classnames(CommonStyle.pageWithPadding, CommonStyle.pageWithColor)}>
      <Card
        title='石头颜色等级表（stone_color_grades）'
        bordered={false}
        actions={
...
export default memo(StoneColorGradesPage);
